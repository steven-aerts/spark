...
object DeserializerBuildHelper {
  ...
  private def createDeserializer(
    ...
    case JavaRecordEncoder(tag, fields) =>
      val cls = tag.runtimeClass
      val dt = ObjectType(cls)
      // For records, we use the canonical constructor with all fields as arguments
